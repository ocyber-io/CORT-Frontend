        ...
        <thead>
          <tr>
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day, i) => (
              <th
                key={`day-${i}`}
                className="py-2 w-[50px] text-[16px] leading-[150%] font-medium"
              >
                {day}
              </th>
            ))}
          ...
          {calendar.map((week, i) => (
            <tr key={`week-${i}`}>
              {week.map((day, j) => (
                <td
                  key={`day-${j}`}
                  className={`py-[12px]  text-center cursor-pointer  hover:bg-datebg hover:text-white hover:rounded-full ${
                    day === today.getDate() &&
                    currentMonth.getMonth() === today.getMonth() &&
                    currentMonth.getFullYear() === today.getFullYear()
                      ? "text-white rounded-full bg-[#BE9A8C]"
                      : ""
                  } ${
